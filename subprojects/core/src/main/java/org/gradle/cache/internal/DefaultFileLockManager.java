...
public class DefaultFileLockManager implements FileLockManager {
    ...
    private class DefaultFileLock extends AbstractFileAccess implements FileLock {
        ...
        private java.nio.channels.FileLock lockStateRegion(final LockMode lockMode, final CountdownTimer timer) throws IOException, InterruptedException {
            return retryUntil(timer, new IOQuery<java.nio.channels.FileLock>() {
                @Override
        ...
        private <T> T retryUntil(CountdownTimer timer, IOQuery<T> query) throws IOException, InterruptedException {
            int iteration = 0;
            T result;
            while ((result = query.run()) == null) {
                ...
                Thread.sleep(backoff.forIteration(++iteration));
            }
            return result;
        }
    }
    ...
    private static class ExponentialBackoff {
        ...
        long forIteration(int i) {
            return random.nextInt(Math.min(i, CAP_FACTOR)) * SLOT_TIME;
        }
    }
}
