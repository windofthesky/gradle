...
public class DownloadingScalaToolChain implements ScalaToolChainInternal {
    ...
    public DownloadingScalaToolChain(File gradleUserHomeDir, File rootProjectDir, WorkerDaemonFactory workerDaemonFactory, WorkerDirectoryProvider workerDirectoryProvider, ConfigurationContainer configurationContainer, DependencyHandler dependencyHandler) {
    ...
    public ToolProvider select(ScalaPlatform targetPlatform) {
        try {
            ...
            return new DefaultScalaToolProvider(gradleUserHomeDir, rootProjectDir, workerDaemonFactory, workerDirectoryProvider, resolvedScalaClasspath, resolvedZincClasspath);
